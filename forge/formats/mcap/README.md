...
## Topic selection — opt-in via YAML

MCAP recordings frequently carry many more topics than a manipulation policy
needs (LIDAR pointclouds, TF trees, diagnostics, parameter events, battery
state). **Forge only loads the topics you explicitly name in `fields:`** —
plus the marker topic and task topic if those are configured. Everything else
is skipped at the MCAP chunk-index level, so unused channels are not decoded
or held in memory.

There is intentionally no exclude-list. To narrow a 200GB recording with
80 topics down to "two cameras + joint states", just list those four entries
in `fields:`:
...
fields:
  ...
  observation.images.front:
    topic: /front_cam/image_raw/compressed
  observation.images.wrist:
    topic: /wrist_cam/image_raw/compressed
  action:
...
`forge inspect <file>.mcap --generate-config out.yaml` proposes a starting
config based on schema heuristics — `JointState` / `Image` / `CompressedImage`
/ `CompressedVideo` / `Pose*` / `Float*MultiArray` topics are picked up;
everything else is silently dropped. Edit the YAML to add or remove fields
as needed before downstream conversion / training.
...
- **No within-message slicing** — you can map an entire `JointState.position`
  to a Forge field but can't yet say "joints 3–7 only". Easy follow-up if
  needed: extend `FieldMapping` with an optional slice/index expression.
